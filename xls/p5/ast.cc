...
absl::Status VisitAll::VisitModule(Module *node) {
  XLS_RETURN_IF_ERROR(VisitStmtBlock(node->body));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitStmtBlock(StmtBlock *node) {
  for (auto stmt_ptr : node->stmts()) {
    XLS_RETURN_IF_ERROR(VisitStmt((Stmt *)stmt_ptr));
  }
  return absl::OkStatus();
}

absl::Status VisitAll::VisitIfStmt(IfStmt *node) {
  XLS_RETURN_IF_ERROR(VisitExpr(node->condition()));
  XLS_RETURN_IF_ERROR(VisitStmt(node->then_block()));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitIfElseStmt(IfElseStmt *node) {
  XLS_RETURN_IF_ERROR(VisitExpr(node->condition()));
  XLS_RETURN_IF_ERROR(VisitStmt(node->then_block()));
...
absl::Status VisitAll::VisitAssignStmt(AssignStmt *node) {
  XLS_RETURN_IF_ERROR(VisitLvalue(node->lhs()));
  XLS_RETURN_IF_ERROR(VisitExpr(node->rhs()));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitExprEvalStmt(ExprEvalStmt *node) {
  XLS_CHECK(node->expr()->GetKind() == AstNodeKind::kBuiltinCallExpr);
  XLS_RETURN_IF_ERROR(VisitBuiltinCallExpr((BuiltinCallExpr *)node->expr()));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitBuiltinCallExpr(BuiltinCallExpr *node) {
  XLS_CHECK(
      node->callee() == "_get_anchor" || node->callee() == "_stack_push_h" ||
      node->callee() == "_stack_push_b" || node->callee() == "_valid_set" ||
      node->callee() == "_valid" || node->callee() == "sizeof");
...
absl::Status VisitAll::VisitNameRefExpr(NameRefExpr *node) {
  return absl::OkStatus();
}

absl::Status VisitAll::VisitFieldAccessExpr(FieldAccessExpr *node) {
  XLS_RETURN_IF_ERROR(VisitLvalue(node->source()));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitArrIndexExpr(ArrIndexExpr *node) {
  XLS_RETURN_IF_ERROR(VisitExpr(node->expr()));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitBitSliceExpr(BitSliceExpr *node) {
  XLS_RETURN_IF_ERROR(VisitExpr(node->target()));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitBinaryOpExpr(BinaryOpExpr *node) {
  XLS_RETURN_IF_ERROR(VisitExpr(node->lhs()));
  XLS_RETURN_IF_ERROR(VisitExpr(node->rhs()));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitUnaryOpExpr(UnaryOpExpr *node) {
  XLS_RETURN_IF_ERROR(VisitExpr(node->operand()));
  return absl::OkStatus();
}

absl::Status VisitAll::VisitCastExpr(CastExpr *node) {
  XLS_RETURN_IF_ERROR(VisitExpr(node->expr_to_cast()));
  XLS_CHECK(node->cast_to()->GetKind() == AstNodeKind::kTypeAnnotation ||
            node->cast_to()->IsLvalueKind());
  if (node->cast_to()->GetKind() == AstNodeKind::kTypeAnnotation) {
    XLS_RETURN_IF_ERROR(VisitTypeAnnotation((TypeAnnotation *)node->cast_to()));
  } else {
    XLS_CHECK(node->cast_to()->GetKind() == AstNodeKind::kNameRefExpr);
    // TODO: ignore typed casting currently
    // XLS_RETURN_IF_ERROR(VisitNameRefExpr((NameRefExpr *)node->cast_to()));
  }
  return absl::OkStatus();
}

absl::Status VisitAll::VisitIntLiteralExpr(IntLiteralExpr *node) {
  return absl::OkStatus();
}

absl::Status VisitAll::VisitLongIntLiteralExpr(LongIntLiteralExpr *node) {
  return absl::OkStatus();
}

absl::Status VisitAll::VisitReturnStmt(ReturnStmt *node) {
  return absl::OkStatus();
}

absl::Status VisitAll::VisitNopStmt(NopStmt *node) { return absl::OkStatus(); }

absl::Status VisitAll::VisitTypeAnnotation(TypeAnnotation *node) {
  return absl::OkStatus();
}

absl::Status VisitAll::VisitFakeVarDef(FakeVarDef *node) {
  return absl::OkStatus();
}

absl::Status VisitAll::VisitVarRefExpr(VarRefExpr *node) {
  XLS_RETURN_IF_ERROR(VisitFakeVarDef(node->def()));
  return absl::OkStatus();
}
