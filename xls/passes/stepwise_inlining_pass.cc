...
absl::Status InlineInvoke(Invoke* invoke, int inline_count) {
  XLS_VLOG(2) << "Inline: " << invoke->ToString();
...
std::vector<FunctionBase*> FunctionsInPostOrder(Package* p) {
  ...
  XLS_VLOG(2) << "Post orders: " << absl::StrJoin(post_order, ", ",
      [](std::string* out, FunctionBase* f) { absl::StrAppend(out, f->name());});
  return post_order;
}

}  // namespace

absl::StatusOr<bool> StepwiseInliningPass::RunInternal(Package* p,
  ...
  for (FunctionBase* f : FunctionsInPostOrder(p)) {
    ...
    for (Node* node : nodes) {
      if (node->Is<Invoke>()) {
        XLS_VLOG(2) << "Inline " << node->As<Invoke>()->to_apply()->name();
